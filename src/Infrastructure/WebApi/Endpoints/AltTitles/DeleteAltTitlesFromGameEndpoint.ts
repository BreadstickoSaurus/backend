...
export class DeleteAltTitlesFromGameEndpoint implements Endpoint{
    private readonly _errorsBag = new ErrorsBag();
    private readonly DeleteAltTitlesFromGame = z.object({
    ...
    async handle(context: RouterContext<string>): Promise<void> {
        try{
            ...
            validateRequest(data, this.DeleteAltTitlesFromGame, this._errorsBag);
            ...
            controller.deleteAltTitlesFromGame(parseInt(gameId), data.altTitles);

            context.response.status = 200;
            context.response.body = { 
                gameId,
                "message": "Alt titles successfully deleted from game"
            };
        }catch(error){
